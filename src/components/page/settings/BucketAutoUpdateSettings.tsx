...
const INTERVAL_OPTIONS: { label: string; value: string; description: string }[] = [
  {
    label: 'settings.bucketAutoUpdate.off',
    value: 'off',
    description: 'settings.bucketAutoUpdate.offDescription',
  },
  {
    label: 'settings.bucketAutoUpdate.every24Hours',
    value: '24h',
    description: 'settings.bucketAutoUpdate.every24HoursDescription',
  },
  {
    label: 'settings.bucketAutoUpdate.everyWeek',
    value: '7d',
    description: 'settings.bucketAutoUpdate.everyWeekDescription',
  },
];
...
export default function BucketAutoUpdateSettings() {
  ...
  const fetchInterval = async () => {
    ...
    setError(null);
    try {
      const value = await invoke<unknown>('get_config_value', {
    ...
    >
      <div class="flex flex-col gap-2">
        {INTERVAL_OPTIONS.map((opt) => (
          <label class="bg-base-200 border-base-300 hover:border-base-content/70 focus-within:border-info focus-within:ring-info/25 flex w-full cursor-pointer items-center justify-between rounded-md border px-3 py-2 transition-colors focus-within:ring-2">
            <div class="flex flex-col">
...
function CustomIntervalEditor(props: CustomIntervalEditorProps) {
  ...
  onMount(() => {
    ...
    if (secs) {
      ...
      if (secs % 604800 === 0) {
        setQuantity(secs / 604800);
        setUnit('weeks');
      } else if (secs % 86400 === 0) {
        setQuantity(secs / 86400);
        setUnit('days');
      } else if (secs % 3600 === 0) {
        setQuantity(secs / 3600);
        setUnit('hours');
      } else if (secs % 60 === 0) {
        setQuantity(secs / 60);
        setUnit('minutes');
  ...
  const unitSeconds = (u: string) =>
    ({ minutes: 60, hours: 3600, days: 86400, weeks: 604800 })[u] || 0;
  ...
  const updatePreview = () => {
    ...
    setPreview(
      t('settings.bucketAutoUpdate.previewFormat', { seconds: secs, human: formatHuman(secs) })
    );
  };
  ...
  const handlePersist = async () => {
    ...
    if (secs < minSecs) {
      setError(`${t('settings.bucketAutoUpdate.intervalTooShort', { seconds: minSecs })}`);
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div class="space-y-2">
      <div class="flex items-end gap-2">
        <div class="flex-1">
          <label class="label">
            <span class="label-text text-xs">{t('settings.bucketAutoUpdate.quantity')}</span>
          </label>
          <input
            type="number"
            min={1}
            ...
            onInput={(e) => {
              setQuantity(parseInt(e.currentTarget.value || '1', 10));
              updatePreview();
            }}
          />
        </div>
...
function ActiveIntervalDisplay(props: { value: string }) {
  ...
  return (
    <div class="bg-base-300 border-base-content/10 rounded border px-2 py-1 text-xs font-medium">
