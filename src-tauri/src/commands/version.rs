...
pub fn check_and_update_version(app: AppHandle) -> Result<bool, String> {
    ...
    if !new_data_dir.exists() {
        fs::create_dir_all(&new_data_dir)
            .map_err(|e| format!("Failed to create new data dir: {}", e))?;
    ...
    if !stored_version.trim().is_empty() && stored_version.trim() == current_version {
        log::trace!(
            "Version {} unchanged from persisted file in new location.",
            current_version
        );
        return Ok(false);
    }
    ...
    if old_version_path.exists() {
        ...
        if old_version.trim() == current_version && stored_version.trim().is_empty() {
            ...
            log::info!(
                "Migrated version {} from old location to new location.",
                current_version
            );
    ...
    log::info!(
        "Detected new version {} (persisted to new location).",
        current_version
    );
    Ok(true)
}
