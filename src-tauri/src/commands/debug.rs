...
fn is_webview_locked_dir(dir_path: &std::path::Path) -> bool {
    if let Some(dir_name) = dir_path.file_name().and_then(|n| n.to_str()) {
        WEBVIEW_LOCKED_DIRS
            .iter()
            .any(|locked_name| dir_name == *locked_name)
    } else {
        false
    }
}
...
pub fn clear_store_data() -> Result<(), String> {
    ...
    let store_files = vec![
        ...
        dirs::data_dir().map(|d| {
            d.join(TAURI_APP_ID)
                .join(format!("{}{}", FRONTEND_STORE_FILE, BACKUP_EXT))
        }),
    ];
    ...
    log::info!(
        "Store cleanup completed. Removed {} files, created marker: {}",
        cleared_count,
        marker_created
    );
        ...
        if cache_dir.exists() {
            ...
            for locked_dir_name in WEBVIEW_LOCKED_DIRS {
                ...
                if locked_dir.exists() && locked_dir.is_dir() {
                    log::info!(
                        "Attempting to remove WebView cache dir: {}",
                        locked_dir.display()
                    );
            ...
            if logs_dir.exists() && logs_dir.is_dir() {
                log::info!(
                    "Attempting to remove logs directory: {}",
                    logs_dir.display()
                );
            ...
            log::info!(
                "WebView cache cleanup completed. Removed {} directories.",
                cleared_dirs
            );
        }
    }
...
pub fn schedule_webview_cleanup() -> Result<(), String> {
    if let Some(app_data_dir) = dirs::data_dir() {
        ...
        if let Some(parent) = marker_file.parent() {
            match fs::create_dir_all(parent) {
                Ok(_) => match fs::write(&marker_file, "Cleanup WebView cache on next startup") {
                    Ok(_) => {
                ...
                Err(e) => log::warn!(
                    "Failed to create directory for WebView cleanup marker: {}",
                    e
                ),
            }
        }
...
pub fn final_cleanup_on_exit() -> Result<(), String> {
    ...
    let final_cleanup_files =
        vec![dirs::data_dir().map(|d| d.join(TAURI_APP_ID).join(FRONTEND_STORE_FILE))];
