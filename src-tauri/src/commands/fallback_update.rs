...
#[tauri::command]
pub async fn check_for_fallback_update(
    _app_handle: AppHandle,
) -> Result<FallbackUpdateInfo, String> {
    ...
    if version == current_version {
        log::info!(
            "Remote version {} is the same as current version {}",
            version,
            current_version
        );
...
async fn get_signature_for_version(_version: &str) -> Result<String, String> {
    ...
    let update_data: serde_json::Value = response
        .json()
        .await
                ...
                if let Some(sig_str) = signature.as_str() {
                    ...
                    match general_purpose::STANDARD.decode(sig_str) {
                        Ok(decoded_bytes) => {
                            ...
                            let hex_string = decoded_bytes
                                .iter()
                                .map(|b| format!("{:02x}", b))
                                .collect::<String>();
                            log::info!(
                                "Successfully decoded signature from Base64: {}...",
                                &hex_string[..32]
                            ); // Show first 32 chars
