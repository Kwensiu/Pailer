 ...
 * @returns true if the line should be displayed as an error
 */
export function isErrorLineWithContext(line: string, previousLines: string[] = [], isStderr?: boolean): boolean {
  ...
  if (isInInstallationProcess) {
    ...
    const isStatusUpdate = statusUpdatePatterns.some(pattern =>
  ...
  const followsSuccess = successIndicators.some(indicator =>
    previousLine.includes(indicator)
  );
  ...
  if (followsSuccess) {
    ...
    const isCleanupRelated = cleanupPatterns.some(pattern =>
      cleanLine.includes(pattern)
    );
  ...
  const hasPreviousError = previousLines.some(prevLine => 
    isErrorLine(prevLine, isStderr)
  );
